...
pub fn unzip_to_memory(input_path_str:String,print_mode:PrintMode)->MemoryImages{
    ...
    for i in 0..archive.len() {
        ...
        if (*file.name()).ends_with('/') {
         if print{   println!("File {} ext \"{}\"", i, outpath.display());}
   ...
   if memory_images.len() > 1 { 
       if memory_images.len() != r_path.len(){print!("len anomaly ImageLen:{} pathLen:{}",memory_images.len(),r_path.len());}
       
       return  MemoryImages{ input_memory_images:memory_images,out_names:r_path,print_mode:print_mode}
   }
...
impl MemoryImages {
    ...
    pub fn convert_size(&self,as_width:u32,as_height:u32,conv_mode:ConvMode) ->  MemoryImages{
        ...
        PrintMode::Print => {print = true; pm = PrintMode::Print},
        PrintMode::Unprint => {print =false; pm = PrintMode::Unprint}
    }
      ...
      return   MemoryImages{ input_memory_images:conv_images,out_names:self.out_names.clone(),print_mode:pm}
    }
